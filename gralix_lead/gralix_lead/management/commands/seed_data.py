...
import random
from gralix_lead.models import Personnel, Lead, Communication, Assignment, Team
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Create a few sample teams (optional cross-division groups)
        teams = [
            Team.objects.get_or_create(name='Tech Growth', defaults={'division': 'tech', 'description': 'Tech sales team'})[0],
            Team.objects.get_or_create(name='Actuarial Growth', defaults={'division': 'actuarial', 'description': 'Actuarial sales team'})[0],
            Team.objects.get_or_create(name='Capital Growth', defaults={'division': 'capital', 'description': 'Capital sales team'})[0],
            Team.objects.get_or_create(name='Enterprise Cross-Division', defaults={'division': None, 'description': 'Cross-division strategic deals'})[0],
        ]
        personnel_data = [
        ...
        for lead_data in leads_data:
            ...
            if assigned_person:
                ...
                # Workload is now derived from DB counts; no mutable counter updates needed.
